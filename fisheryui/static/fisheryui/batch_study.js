...
/*
 * Calculates and updates the steps and combos for the batch study.
 */
function updateStats() {
    var tr_elements = {};
    var n_steps = 0, tot_steps = 0, combos = 0, tot_combos = 0, default_combos = 0;
    ...
    for (i = 0; i < tr_elements.length; i++) {
        if ($(tr_elements[i]).find("td#setting_default input")[0].checked) {
            default_combos = calculateCombos(
    ...
    /* Get all included settings. */
    for (i = 0; i < tr_elements.length; i++) {
        /* If setting is included and not the default sweep setting. */
        if ($(tr_elements[i]).find("td#setting_included input")[0].checked &&
            ...
            combos = calculateCombos(
                ...
                parseInt($(tr_elements[i]).find("td#setting_end input")[0].value),
                parseInt($(tr_elements[i]).find("td#setting_int input")[0].value)).length;
            /* Update total amount of steps / combinations. */
            tot_steps += n_steps * combos * default_combos;
            tot_combos += combos * default_combos;
        }
    }
